 ...
 * in a sleepable softirq-deferred context via mod_timer, and we
 * also call get_task_mm / mmput which may sleep. A spinlock would
 * be unsafe here.
 * ═══════════════════════════════════════════════════════════════ */
...
static void timer_callback(struct timer_list *t)
{
    ...
    list_for_each_entry_safe(entry, tmp, &monitored_list, list) {
        ...
        if (rss < 0) {
            printk(KERN_INFO
                   "[container_monitor] PID %d (%s) no longer exists, removing.\n",
...
static int __init monitor_init(void)
{
    ...
    /* ── TODO 2 (init): list and lock are statically initialised
       via LIST_HEAD and DEFINE_MUTEX above — nothing extra needed. */
